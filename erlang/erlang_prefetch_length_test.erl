%% Companion erlang program to blog post "Memory matters - even in Erlang"
%%  http://www.lshift.net/blog/2010/02/28/memory-matters-even-in-erlang


%% erlc erlang_prefetch_length_test.erl && erl -noshell -run erlang_prefetch_length_test start -run init stop;
...
-module(erlang_prefetch_length_test).
-export([start/0]).
-export([loop/1]).

start() ->
    Pid = spawn(erlang_prefetch_length_test, loop, [none]),
    Pid ! init,
    Pid ! len,
    Pid ! len,
    Pid ! len,
...
items(I) ->
    lists:map(fun(_) -> {{ppp,{resource,<<"1">>,exchange,<<"">>},<<"aaaa">>,
                              {content,60,
                                {'P_basic',<<"asdfghjklp">>,undefined,undefined,2,undefined,
                                 undefined,undefined,undefined,undefined,undefined,undefined,
                                 undefined,undefined,undefined
                                },
                               <<"1234567890olki">>,[<<"123212321232">>]
                              },{{4,list_to_pid("<0.202.0>")},22771}
                         },false}
            end,
            lists:seq(0, I)).
...
loop(Queue) ->
    receive
        ...
        hibernate ->
            ...
            erlang:hibernate(erlang_prefetch_length_test, loop, [Queue]);
        {From, ping} ->
            From ! pong,
            loop(Queue);
        _ -> ok
