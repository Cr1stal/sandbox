...
public class HeapPage implements Page {
    ...
    */
    ...
    public static HeapPage createEmptyPage(HeapPageId pid) {
        HeapPage p;
        try {
            p = new HeapPage(pid, createEmptyPageData());
        } catch (IOException e) {
    ...
    public void addTuple(Tuple t) throws DbException {
        ...
        if (!td.equals(t.getTupleDesc()))
            throw new DbException("tupledesc is mismatch, expected " + td + ", but was " + t.getTupleDesc());
        
        int idx = next_empty_slot();
        t.setRecordId(new RecordId(pid, idx));
        tuples[idx] = t;    // may need to defensively copy t
        setSlot(idx, true);
    }

    private int next_empty_slot() throws DbException {
        for (int i = 0; i < numSlots; ++i)
            if (!getSlot(i))
                return i;
        
        // should next reach here
