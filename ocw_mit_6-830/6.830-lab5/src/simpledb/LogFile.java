...
public class LogFile {
    ...
    public void rollback(TransactionId tid)
        ...
        synchronized (Database.getBufferPool()) {
            synchronized(this) {
                ...
                Long first_offset = tidToFirstLogRecord.get(tid.getId());
                if (first_offset == null)
                    throw new NoSuchElementException("Transaction doesn't exist: " + tid.getId());
                raf.seek(first_offset);
                while (true) {
                    try {
                        ...
                        case UPDATE_RECORD:
                            ...
                            break;
                        case CHECKPOINT_RECORD:
                            int xcount = raf.readInt();
                            raf.skipBytes(xcount * 2 * LONG_SIZE);  // a tid + an offset each
                            break;
                        }
                        raf.readLong(); // read off offset of the record
                    } catch (EOFException e) {
                        break;
                    }
                }
    ...
    */
    private class XactionOffsetPair {
        long    xaction_id;
        long    log_offset;
        XactionOffsetPair(long tid, long offset) {
            xaction_id = tid;
            log_offset = offset;
        }
    };
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                Set<Long> loser_xactions = new HashSet<Long>();
                Map<PageId, XactionOffsetPair> page_to_last_xaction = new HashMap<PageId, XactionOffsetPair>();
                raf.seek(LONG_SIZE);
                while (true) {
                    try {
                        int record_type = raf.readInt();
                        ...
                        switch (record_type) {
                        case COMMIT_RECORD:
                            loser_xactions.remove(record_tid);
                            raf.readLong();
                            break;
                        case BEGIN_RECORD:
                        case ABORT_RECORD:
                            loser_xactions.add(record_tid);
                            raf.readLong();
                            break;
                        case UPDATE_RECORD:
                            // redo
                            ...
                            page_to_last_xaction.put(pid, new XactionOffsetPair(record_tid, offset));
                            break;
                        case CHECKPOINT_RECORD:
                            int xcount = raf.readInt();
                            for (int i = 0; i < xcount; i++) {
                                long xid = raf.readLong();
                                loser_xactions.add(xid);
                                raf.readLong();
                            }
                            raf.readLong();
                            break;
                ...
                // undo
                for (XactionOffsetPair pair : page_to_last_xaction.values()) {
                    if (loser_xactions.contains(pair.xaction_id)) {
                        raf.seek(pair.log_offset + INT_SIZE + LONG_SIZE);
                        Page before = readPageData(raf);
                        Database.getCatalog()
                                .getDbFile(before.getId().getTableId())
                                .writePage(before);
                    }
                }
            }
